...
# parameters
player_stats = {}
batting_stats = ['G', 'AB', 'R', 'H', '2B', '3B', 'HR', 'RBI', 'BB', 'IBB', 'HBP', 'SH', 'SF']
...
for player_season in player_positions:  # remove players who played >90% games as pitchers or catchers
    for position in positions_to_remove:
        if position in player_positions[player_season] and \
                float(player_positions[player_season][position]) / sum(player_positions[player_season].values()) > 0.9:
...
# drop players with no ABs
stats_df = stats_df[stats_df['AB'] > 0]
logging.info('NUMBER OF PLAYER SEASONS WITH 1+ AB\'s: %d', len(stats_df))

# drop players before 1933 (first all-star game in 1933)
stats_df = stats_df[stats_df['yearID'] >= 1933]
logging.info('NUMBER OF PLAYER SEASONS 1933 OR LATER: %d', len(stats_df))
...
# replace NaN values (missing data in raw files) with 0s for IBB, HBP, SH, SF
...
# re-order columns
stats_df = stats_df[['playerID', 'yearID', 'G', 'AB', 'R', 'H', '2B', '3B', 'HR', 'RBI', 'BB', 'IBB', 'HBP', 'SH',
                     'SF', 'AVG', 'OBP', 'slugging', 'OPS', 'G_field', 'GS_field', 'InnOuts_field', 'PO_field',
                     'A_field', 'E_field', 'n_awards', '1933-1941', '1942-1945', '1946-1962', '1963-1976',
